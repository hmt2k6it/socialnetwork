...
public class RoleControllerUnitTest {
    ...
    @Test
    void assignPermissionToRole_success() throws Exception {
        when(roleService.assignPermissionToRole(anyString(), any())).thenReturn(response);

        mockMvc.perform(put("/api/v1/roles/{roleName}", "USER")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))
        );
    }
